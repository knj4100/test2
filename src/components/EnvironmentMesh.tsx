...
function EnvironmentMeshInner({ url, position, scale, visible, opacity = 1, onGroundDetected, vehiclePosition, onContactY }: EnvironmentMeshProps) {
  ...
  // Continuous Ground Following (throttled + position-change check)
  const frameCounter = useRef(0);
  const lastVehiclePos = useRef({ x: NaN, z: NaN });
  const rayOrigin = useMemo(() => new THREE.Vector3(), []);

  useFrame(() => {
    if (!vehiclePosition || !onContactY || !meshRef.current) return;

    // Skip if vehicle hasn't moved
    const dx = vehiclePosition.x - lastVehiclePos.current.x;
    const dz = vehiclePosition.z - lastVehiclePos.current.z;
    if (dx * dx + dz * dz < 0.0000001 && !isNaN(lastVehiclePos.current.x)) return;

    // Throttle: raycast every 5 frames
    frameCounter.current++;
    if (frameCounter.current % 5 !== 0) return;

    lastVehiclePos.current.x = vehiclePosition.x;
    lastVehiclePos.current.z = vehiclePosition.z;

    // Reuse pre-allocated Vector3
    rayOrigin.set(vehiclePosition.x, 10000, vehiclePosition.z);
...
export const EnvironmentMesh = React.memo(function EnvironmentMesh(props: EnvironmentMeshProps) {
  if (!props.url) {
    console.warn('[EnvironmentMesh] Model URL is empty or not configured.');
    return null;
  }
  return <EnvironmentMeshInner {...props} />;
});
